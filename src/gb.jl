...
function writeGB(outfile_gb::String, uid::UUID, id::AbstractString, gffs::Vector{GFF})
    open(outfile_gb, "w") do out
        writeGB(out, uid, id, gffs)
    end
end
function writeGB(out::IO, uid::UUID, id::AbstractString, gffs::Vector{GFF})
    ...
    for gff in gffs
        ...
        if gff.ftype == "CDS"
            ...
            skip3("protein_id", "gnl|Chloe2|$(uuid5(uid, attributes["Name"]))")
